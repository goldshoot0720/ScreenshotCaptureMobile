...
class FloatingCaptureService : Service() {
    ...
    private fun showBubbles() {
        ...
        addBubble(R.drawable.ic_capture_shutter, PINK, PURPLE, dp(17), dp(64), 0, 0, "擷取整個螢幕", tint = false) { capture() }
        addBubble(R.drawable.ic_return, PURPLE, INDIGO, dp(15), dp(52), dp(6), dp(74), "返回螢幕擷取 App", tint = false) {
        ...
        addBubble(R.drawable.ic_close, SLATE, SLATE_DARK, dp(16), dp(52), dp(6), dp(136), "關閉懸浮擷取按鈕", tint = false) { shutdown() }
    }

    /** [tint] paints the icon white, which suits the flat glyphs but would flatten a full-colour one. */
    private fun addBubble(icon: Int, from: Int, to: Int, inset: Int, size: Int, offsetX: Int, offsetY: Int, description: String, tint: Boolean = true, onClick: () -> Unit) {
        val view = ImageView(this).apply {
            setImageDrawable(ContextCompat.getDrawable(this@FloatingCaptureService, icon))
            if (tint) setColorFilter(Color.WHITE)
            scaleType = ImageView.ScaleType.FIT_CENTER; contentDescription = description
    ...
    private fun notification(): android.app.Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID).setSmallIcon(R.drawable.ic_notification).setContentTitle(getString(R.string.app_name)).setContentText("懸浮擷取按鈕已啟用").setOngoing(true).build()
    }
